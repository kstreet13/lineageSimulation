...
library(fastICA)
#library(Rtsne)
simResults_slingICA <- mclapply(1:nIts, function(i){
...
save(simResults_slingICA, file='temp_slingICA.RData')

# format output
nlins <- t(sapply(simResults_slingICA, function(res){
  res[1,]
}))
taus <- lapply(1:ncol(simResults_slingICA[[1]]),function(i){
  t(sapply(simResults_slingICA, function(res){
    res[-1, i]
  }))
})
names(taus) <- colnames(simResults_slingICA[[1]])

save(nlins,taus, file='results_slingICA.RData')
