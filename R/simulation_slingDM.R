...
#library(fastICA)
#library(Rtsne)
simResults_slingDM <- mclapply(1:nIts, function(i){
...
save(simResults_slingDM, file='temp_slingDM.RData')

# format output
nlins <- t(sapply(simResults_slingDM, function(res){
  res[1,]
}))
taus <- lapply(1:ncol(simResults_slingDM[[1]]),function(i){
  t(sapply(simResults_slingDM, function(res){
    res[-1, i]
  }))
})
names(taus) <- colnames(simResults_slingDM[[1]])

save(nlins,taus, file='results_slingDM.RData')
