...
taus <- lapply(1:ncol(simResults_dpt5[[1]]),function(i){
  t(sapply(simResults_dpt5, function(res){
    res[-1, i]
  }))
})
names(taus) <- colnames(simResults_dpt5[[1]])

save(nlins,taus, file='results_dpt5.RData')
